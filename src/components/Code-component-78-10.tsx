...
export function EnhancedVideoPlayer({ output, screenplay, onClose, onDownload }: EnhancedVideoPlayerProps) {
  ...
  const handleDownload = () => {
    ...
    if (url) {
      try {
        ...
        let filename = `reelchemist_${screenplay?.title?.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase() || 'film'}_${sceneSlug}`;
        ...
        toast.success('🚀 Download Started', {
          description: `Downloading "${filename}" from your ${screenplay?.title || 'film'} production!`
        });
  ...
  const handleShare = () => {
    if (navigator.share && output.url) {
      navigator.share({
        title: `${screenplay?.title || 'ReelChemist'} - ${output.character || output.scene || 'Asset'}`,
        text: `Check out this cinematic asset from my ${screenplay?.title || 'film'} production!`,
        url: output.url
      }).catch(() => {
  ...
  const getRelevantSceneInfo = () => {
    ...
    const dialogue = screenplay.dialogue?.filter((d: any) => d.scene === output.scene);
    const characters = screenplay.characters?.filter((c: any) => 
      scene?.characters?.includes(c.name) || dialogue?.some((d: any) => d.character === c.name)
    );
    ...
    return (
      <div className="space-y-4">
        <h4 className="font-semibold flex items-center space-x-2">
          ...
          <span>Scene Context from "{screenplay?.title}"</span>
        </h4>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                {screenplay?.title && (
                  <div className="text-center p-3 glass-effect rounded-lg">
                    ...
                    <div className="text-xs text-foreground truncate">{screenplay.title}</div>
                  </div>
                )}
              </div>
            </div>
